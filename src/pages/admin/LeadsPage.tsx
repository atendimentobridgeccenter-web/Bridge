...
/** Resolve title de uma node pelo ID (fallback: "Pergunta #n") */
function resolveTitle(nodeId: string, nodes: FormNode[]): string {
  const node = nodes.find(n => n.id === nodeId)
  return node?.title || `Campo ${nodeId.slice(0, 6)}`
}

/** Extrai nome de exibição do lead: coluna name > answers */
...
/** Gera cor de badge a partir do nome do produto */
...
function productColor(name: string | null | undefined) {
  ...
  const idx = name.split('').reduce((a, c) => a + c.charCodeAt(0), 0) % PRODUCT_COLORS.length
...
function SelectFilter({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {value ? (
          <button onClick={e => { e.stopPropagation(); onChange('') }}
            className="ml-auto shrink-0 text-white/25 hover:text-white/60 transition-colors">
            <X className="w-3 h-3" />
          </button>
        ) : (
      ...
      {open && (
        <div
          className="absolute top-full left-0 mt-1.5 rounded-xl z-50 py-1.5 min-w-[200px]"
        ...
        >
          {options.map(opt => (
            <button key={opt.value}
              ...
              className={cn(
                'w-full flex items-center px-4 py-2 text-[12px] text-left transition-colors',
...
function LeadDrawer({ lead, onClose }: { lead: Lead; onClose: () => void }) {
  ...
  // Filtra respostas significativas (remove UUIDs sem match que sejam vazias)
        ...
        <div className="flex items-start justify-between px-6 py-5 shrink-0"
          ...
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {name && lead.email && (
                <p className="text-[12px] text-white/35 mt-0.5 truncate">{lead.email}</p>
              )}
              <p className="text-[11px] text-white/25 mt-1">{fmtDate(lead.created_at)}</p>
            </div>
          </div>
          <button onClick={onClose}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-white/30 hover:text-white/70 hover:bg-white/6 transition-all shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2">
                {answersEntries.map(([nodeId, answer], idx) => {
                  const title = resolveTitle(nodeId, nodes)
                  return (
                    <div key={nodeId}
  ...
  return (
    ...
    >
      ...
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            {name ? (
              <>
                <p className="text-[13px] font-semibold text-[#EDEDED] leading-none truncate">{name}</p>
                {lead.email && <p className="text-[11px] text-white/30 mt-0.5 truncate">{lead.email}</p>}
              </>
            ) : (
              <p className="text-[13px] text-white/50 truncate">{lead.email ?? '—'}</p>
            )}
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-3.5 text-right" style={{ width: 80 }}>
        <button
          onClick={onView}
          className="opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium ml-auto"
          style={{ background: 'rgba(232,82,26,0.1)', color: '#E8521A', border: '1px solid rgba(232,82,26,0.2)' }}
          onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(232,82,26,0.18)' }}
          onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'rgba(232,82,26,0.1)' }}
        >
...
export default function LeadsPage() {
  ...
  const { data: leads = [], isLoading, error } = useLeads(productId || null)
  ...
  function refresh() { qc.invalidateQueries({ queryKey: ['leads'] }) }
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: BG_CARD, border: `1px solid ${BORDER}` }}>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                ...
                <button onClick={refresh}
                  className="text-[12px] text-[#E8521A] hover:underline">
                  Tentar novamente
                </button>
              </div>
            ) : filtered.length === 0 ? (
                ...
                <div className="text-center">
                  ...
                  {(search || productId) && (
                    <p className="text-[12px] text-white/20 mt-1">
                      ...
                      <button className="text-[#E8521A] hover:underline" onClick={() => { setSearch(''); setProductId('') }}>
                        limpe a busca
                      </button>
                    </p>
                  )}
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      {['Contato', 'Telefone', 'Produto', 'Status', 'Data', ''].map((h, i) => (
                        <th key={i}
                          className="text-left px-5 py-3.5 text-[11px] font-semibold uppercase tracking-wider"
                ...
                <div className="px-5 py-3 flex items-center justify-between"
                  ...
                  <p className="text-[10px] text-white/15">
                    Atualizados em tempo real via Supabase
                  </p>
                </div>
              </div>
            )}
