...
export default function CheckoutTab({ product, onChange }: Props) {
  ...
  return (
    <div className="max-w-xl space-y-8">
        ...
        <div className="space-y-4">
          <Field label="Nome do produto">
            <input
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-colors"
            />
          </Field>
          <Field label="Slug (URL)">
            <div className="flex items-center gap-0 rounded-xl border border-slate-700 overflow-hidden focus-within:border-violet-500 transition-colors">
              <span className="px-3 py-2.5 bg-slate-800/80 text-slate-500 text-sm border-r border-slate-700">/product/</span>
              <input
                value={product.slug}
                onChange={e => onChange({ slug: e.target.value.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') })}
                className="flex-1 bg-slate-800 px-3 py-2.5 text-white outline-none text-sm"
              />
            </div>
          </Field>
          <Field label="Descrição curta">
            <textarea
              ...
              rows={3}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 transition-colors resize-none text-sm"
            />
          </Field>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <Field label="Price ID do Stripe" hint="Exemplo: price_1OaXXXXX">
            <input
              ...
              placeholder="price_..."
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white outline-none focus:border-violet-500 font-mono text-sm transition-colors placeholder:text-slate-600"
            />
          </Field>
      ...
      <div className="rounded-2xl border border-violet-500/20 bg-violet-500/5 p-4 text-sm text-slate-400">
